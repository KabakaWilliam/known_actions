...
def load_open_set_auroc(traces_dir: Path, loo_subdir: str = "2wikimultihop_open_set") -> dict[str, dict]:
    ...
    if not loo_dir.exists():
        sys.exit(f"ERROR: open-set directory not found at {loo_dir}")
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=None,
                        help="Output PNG path (default: traces/classifiers/open_set_summary[_scatter].png).")
    parser.add_argument("--plot", choices=["both", "bars", "scatter"], default="both",
                        help="Which panel(s) to produce: both (default), bars (Panel A only), "
                             "scatter (Panel B only).")
