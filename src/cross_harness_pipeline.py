...
def _load_examples(
    rows: list[dict[str, Any]], include_sequences: bool
...
def _model_dir(
    cfg: dict[str, Any], dataset: str, train_policy: str, seed: int, classifier: str
) -> Path:
    return (
        cfg["experiment"]["artifact_root"]
        / "models"
...
def train_model(
    ...
    *,
    ...
    model_dir = _model_dir(cfg, dataset, train_policy, seed, classifier)
...
def evaluate_model(
    ...
    *,
    ...
    include_sequences = classifier == "LSTM"
    X, sequences, labels, feature_names = _load_examples(rows, include_sequences)
...
def run_harness_detector_lomo(
    ...
    *,
    ...
    detector_dir = _harness_detector_dir(cfg, dataset, classifier, seed)
    ...
    for fold_index, held_out_agent in enumerate(cfg["agents"]):
        ...
        X_train, _, _, feature_names = _load_examples(train_rows, False)
        X_val, _, _, val_feature_names = _load_examples(val_rows, False)
        X_test, _, _, test_feature_names = _load_examples(test_rows, False)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config", type=Path, default=Path(__file__).with_name(
            "cross_harness_config.yaml"
        )
    )
