...
Local models are served one at a time through vLLM. Cloud models are run in
the configured order. The existing orchestrator remains responsible for
...
class CampaignRunner:
    ...
    def collection_progress(self, agent_id: str) -> tuple[int, int]:
        ...
        for dataset in self.datasets:
            counts = orchestrator.find_completed_counts(
                agent_id, dataset["name"], self.harness
            )
    ...
    def _assert_gpus_available(self, spec: dict[str, Any]) -> None:
        requested = [int(gpu) for gpu in spec["vllm"].get("gpus", [])]
        maximum_used = int(
            spec["vllm"].get(
    ...
    def _wait_for_vllm(
        self, port: int, served_model: str, startup_timeout_s: float
    ) -> None:
        deadline = time.monotonic() + startup_timeout_s
        last_error = "server has not answered"
        api_key = os.getenv("VLLM_API_KEY") or "not-needed"
        while time.monotonic() < deadline:
            if self.vllm_process is not None and self.vllm_process.poll() is not None:
                raise CampaignError(
                    f"vLLM exited during startup with code "
            ...
            try:
                ...
                model_ids = {
                    item.get("id") for item in response.get("data", [])
                }
            ...
            time.sleep(5)
        raise CampaignError(
            f"vLLM did not become ready within {startup_timeout_s:g}s: "
            f"{last_error}"
        )

    def _smoke_test_vllm(self, port: int, served_model: str) -> None:
        api_key = os.getenv("VLLM_API_KEY") or "not-needed"
        self._json_request(
            ...
            timeout=float(self.settings.get("vllm_smoke_timeout_s", 120)),
        )

    def start_vllm(self, agent_id: str, spec: dict[str, Any]) -> None:
        vllm = spec["vllm"]
        self._assert_gpus_available(spec)
        port = int(vllm["port"])
        ...
        env["CUDA_VISIBLE_DEVICES"] = ",".join(
            str(gpu) for gpu in vllm.get("gpus", [])
        )
        command = [
            str(
                vllm.get(
            ...
            "--port",
            str(port),
            *[str(value) for value in vllm.get("args", [])],
        ]
        log_path = self.run_dir / "logs" / f"{agent_id}.vllm.log"
        log_path.parent.mkdir(parents=True, exist_ok=True)
        self.vllm_log_handle = log_path.open("w")
        self.vllm_process = subprocess.Popen(
            command,
            cwd=PROJECT_DIR,
            env=env,
            stdout=self.vllm_log_handle,
        ...
        self._wait_for_vllm(
            port,
            str(vllm.get("served_model_name", vllm["model"])),
            float(vllm.get("startup_timeout_s", 1800)),
        )
        if vllm.get("smoke_test", True):
            self._smoke_test_vllm(
                port, str(vllm.get("served_model_name", vllm["model"]))
            )

    @staticmethod
    def _terminate_process(process: subprocess.Popen | None, grace_s: float = 20) -> None:
    ...
    def stop_vllm(self) -> None:
        self._terminate_process(self.vllm_process)
        self.vllm_process = None
        if self.vllm_log_handle is not None:
            self.vllm_log_handle.close()
            self.vllm_log_handle = None
    ...
    def _usage_summary(self, agent_id: str) -> dict[str, Any]:
        ...
        trace_root = PROJECT_DIR / "traces" / agent_id
        for path in trace_root.glob(f"*/{self.harness}/*/*.json"):
            try:
                ...
                totals["prompt_tokens"] += int(
                    usage.get("total_prompt_tokens", 0)
                )
    ...
    def run_local(self, spec: dict[str, Any]) -> None:
        ...
        collected, expected = self.collection_progress(agent_id)
        print(f"[LOCAL] {agent_id}: {collected}/{expected} valid traces")
        ...
        if self.dry_run:
            print(
                f"[DRY-RUN] would start {spec['vllm']['model']} on GPUs "
                f"{spec['vllm'].get('gpus')} and collect with "
                f"{self.settings.get('local_workers', 10)} workers"
            )
            return
        configured_url = env.get("MIDSCENE_MODEL_BASE_URL", "")
        expected_port = f":{spec['vllm']['port']}/"
        if expected_port not in configured_url:
            raise CampaignError(
                f"{agent_id} registry URL {configured_url!r} does not match "
                f"campaign port {spec['vllm']['port']}"
            )
        self._set_model_state(agent_id, kind="local", status="starting_vllm")
        try:
            self.start_vllm(agent_id, spec)
            self._set_model_state(agent_id, status="collecting")
            self.collect_model(
                agent_id, int(self.settings.get("local_workers", 10))
            )
            self._set_model_state(
                agent_id,
                accounting=self._usage_summary(agent_id),
